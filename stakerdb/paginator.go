...
// file copied from: https://github.com/lightningnetwork/lnd/blob/master/channeldb/paginate.go
...
func (p paginator) keyValueForIndex(index uint64) ([]byte, []byte) {
	var keyIndex [8]byte
	binary.BigEndian.PutUint64(keyIndex[:], index)
...
func (p paginator) lastIndex() uint64 {
	...
	return binary.BigEndian.Uint64(keyIndex)
}
